...
TRAJECTORY_BUILDER_2D.min_range = 0.15 --최소범위
...
TRAJECTORY_BUILDER_2D.voxel_filter_size = 0.03 --0.025 -- 데이터 처리를 얼마나 빠르게 할건지 클수록 빨라짐
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.max_length = 0.7 --0.5 --정한 수보다 큰 길이 제외
...
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 5. --각 디바이스가 측정한 점들의 위치를 얼마만큼 신뢰하는지에 대한 가중치값
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 29. --측정한 점들의 회전정도를 얼만큼 신뢰하는 지에 대한 가중치 
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.ceres_solver_options.use_nonmonotonic_steps = false
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.ceres_solver_options.max_num_iterations = 17 --20
...
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 200 -- 300 -- 1개의 서브맵을 만들때 사용할 라이다 개수
...
POSE_GRAPH.optimize_every_n_nodes = 70  --90
...
POSE_GRAPH.constraint_builder.ceres_scan_matcher.translation_weight = 20 --10.
POSE_GRAPH.constraint_builder.ceres_scan_matcher.rotation_weight = 35 --1.
